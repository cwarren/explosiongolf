...
var COLOR_CHARGE_OUTLINE = makeColor(.8,.8,.8,.2);
...
Circ.createFromString = function(str) {
    var ptMatch = str.match(/\<([^\>]*)\>/);
    var xy = ptMatch[1].split(',');
    
    var radiusMatch = str.match(/\>\#(\d+)/);
//    console.log('radiusMatch is '+radiusMatch);
    
    var colorMatch = str.match(/rgba\([^\)]*\)/);
//    console.log('colorMatch is '+colorMatch);
    return new Circ(new Point2D(xy[0]*1,xy[1]*1),radiusMatch[1]*1,colorMatch[0]);
}
...
Circ.prototype.drawStroke = function (relToPt,strokeThickness) {
    if (relToPt===undefined) {
        relToPt = PT_ORIGIN;
    }
    ...
    strokeCircle(this.loc.x+relToPt.x,this.loc.y+relToPt.y,this.radius,this.color,strokeThickness);
}
...
Polygon.createFromString = function(str) {
    var ptsMatch = str.match(/\<([^\>]*)\>/);
    var ptData = ptsMatch[1].split(';');
    var pts = [];
    for (var i=0;i<ptData.length;++i) {
        var xy = ptData[i].split(',');
        pts.push(new Point2D(xy[0],xy[1]));
    }
    
    var colorMatch = str.match(/rgba\([^\)]*\)/);
//    console.log('colorMatch is '+colorMatch);
    return new Polygon(pts,colorMatch[0]);
}
...
Polygon.prototype.drawFill = function (optional_ref_pt) {
    ...
    //console.log('drawFill of a polygon');
...
Polygon.prototype._getContextWithPath = function(offsetX,offsetY) {
    ...
    if (this.vertices.length > 0) {
        context.moveTo((this.vertices[0].x*1)+(offsetX*1),(this.vertices[0].y*1)+(offsetY*1));
        //console.log('path begins at '+(this.vertices[0].x)+','+(this.vertices[0].y));
        //console.log(' using offsets '+(offsetX)+','+(offsetY));
        var i;
        for (i=1;i<this.vertices.length;i++) {
            context.lineTo((this.vertices[i].x*1)+(offsetX*1),(this.vertices[i].y*1)+(offsetY*1));
        }
        context.lineTo((this.vertices[0].x*1)+(offsetX*1),(this.vertices[0].y*1)+(offsetY*1));
    }
    context.closePath();
...
Polygon.prototype.getAdjustedSplineControlPoints = function (refx,refy) {
    ...
    for (var i=1;i<splineCtrls.length;i+=2) {
        splineCtrls[i-1] = splineCtrls[i-1]*1 + refx*1;
        splineCtrls[i] = splineCtrls[i]*1 + refy*1;
    }
    return splineCtrls;
}
...
Point2D.prototype.limitToPolygon = function(poly) {
    if (poly.containsPt(this)) { return; }
    
    var inter = Intersection.intersectLinePolygon(this,poly.getCenter(),poly.vertices);
    if (inter.points.length > 0) {
        this.x = inter.points[0].x;
        this.y = inter.points[0].y;
    }
}
